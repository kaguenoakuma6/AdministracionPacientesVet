...
export default function PatientForm() {
    ...
    const addPatient = usePatiensStore(state => state.addPatient);
    const activeId = usePatiensStore(state => state.activeId);
    const patients = usePatiensStore(state => state.patients);
    const updatePatient = usePatiensStore(state => state.updatePatient);
          ...
          <form className="bg-white shadow-md rounded-lg py-10 px-5 mb-10" noValidate onSubmit={handleSubmit(registerPatient)}>
                ...
                <div className="mb-5">
                    ...
                    <textarea id="symptoms" className="w-full p-3  border border-gray-100" placeholder="Síntomas del paciente"
                        {...register('symptoms',{ required: 'La descripción de los síntomas son obligatorios'})}
                    />
                    {errors.symptoms && (
